...
if (isset($_POST["n_registros"])) {
    ...
    // Siempre que haya un cambio nos vamos a la página 1 para evitar que se nos quede vacío en una página
    $_SESSION["pag"] = 1;
}
...
<form id='regis_busc' action='index.php' method='post'>
    <div>
        <label for='n_registror'>Registros a mostrar:</label>
        <select name="n_registros" id="n_registros" onchange="document.getElementById('regis_busc').submit();"> <!-- Cuando cambie, hacemos un submit -->
...
if($n_paginas>1)
{
    echo "<form action='index.php' method='post' id='paginas'>";
    echo "<p>";
    if($_SESSION["pag"]!=1)
    {
    ...
    {
        if($i==$_SESSION["pag"])
            echo "<button disabled type='submit' name='btnPagina' value='".$i."'>".$i."</button>"; // Si estamos en la misma página ponemos el botón en disabled (no se puede clickar y tiene un color más claro)
        else
