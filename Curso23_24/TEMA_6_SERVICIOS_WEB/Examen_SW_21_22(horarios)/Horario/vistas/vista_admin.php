<?php 
    ...
    // Vamos primero a sacar todos los profesores
    ...
    // Si existe profesor (se le ha dado al botón ver horario) mostramos el horario de ese profesor
    if (isset($_POST["profesor"])) {
        ...
        if (isset($obj2->error)) {
            session_destroy();
            die(error_page("Examen SW 21_22", "Ha habido un error al consumir el servicio: ".$obj2->error));
        }
...
<body>
    ...
    <?php 
        ...
        echo "<form action='index.php' method='post'>";
            echo "<label for='profesor'>Seleccione el profesor: </label>";
            echo "<select name='profesor' id='profesor'>";
                foreach ($obj->usuarios  as $tupla) {
                    if ($tupla->tipo == "normal") { // Mostramos solo los profesores normales
                        if ($tupla->id_usuario == $_POST["profesor"]) {
                            $nombre_profesor = $tupla->nombre; // Guardamos el nombre del profesor
                            echo "<option selected value='".$tupla->id_usuario."'>".$tupla->nombre."</option>"; // Mantenemos los valores
                        } else {
        ...
        if (isset($_POST["profesor"])) {
            ...
            // Mostramos el horario del profesor seleccionado
            $dias[] = "";
            $dias[1] = "Lunes";
            $dias[2] = "Martes";
            $dias[3] = "Miércoles";
                ...
                if ($hora == 4) { // Si estamos en el recreo
                    ...
                    for ($dia=1; $dia <= 5; $dia++) { 
                        if (isset($horario[$dia][$hora])) {
                            echo "<td>".$horario[$dia][$hora]."<br><button name='btnEditar' class='enlace'>Editar</button></td>";
                        } else {
                            echo "<td><button name='btnEditar' class='enlace'>Editar</button></td>";
                        }                        
                    }
