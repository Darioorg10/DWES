...
if(isset($_POST["btnAgregar"]))
{
    ...
    if(!$error_referencia)
    {
        ...
        $url = DIR_SERV . "/repetido/libros/referencia/".urlencode($_POST["referencia"]); // No hace falta el urlencode, pero si lo ponemos no pasa nada
        $respuesta = consumir_servicios_REST($url, "GET", $datos); // Aquí el $datos trae la api_session (que es lo que nos hace falta) desde seguridad        
        ...
        $error_referencia = $obj->repetido; // Nos devolvía true o false        
    ...
    // Si no hay error insertamos el libro
    if(!$error_form)
    {
...
<body>
    ...
    <?php
        ...
        foreach($obj->libros as $tupla)
        {
            ...
            echo "<td><form action='gest_libros.php' method='post'><button class='enlace' name='btnBorrar' value='".$tupla->referencia."'>Borrar</button> - <button class='enlace' name='btnEditar' value='".$tupla->titulo."'>Editar</button></form></td>";
            echo "</tr>";
        }
        echo "</table>";
