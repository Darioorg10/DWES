        ...
        .done(function (data) {
            if (data.mensaje_error) {
                ...
                $.each(data.productos, function (key, tupla) {
                    ...
                    html_tabla_prod += "<td><button class='enlace' onclick='confirmar_borrar(\"" + tupla["cod"] + "\");'>Borrar</button> - <button class='enlace' onclick='montar_form_editar(\"" + tupla["cod"] + "\")'>Editar</button></td>";
                    html_tabla_prod += "</tr>";
                });
...
function montar_form_editar(cod) {
    $.ajax({
        url: encodeURI("servicios_rest_key/producto/" + cod),
        type: "GET",
        dataType: "json"
    })
        .done(function (data) {
            if (data.mensaje_error) {
                ...
                continuar_montar_form_editar(data.producto);
            }
        })
        .fail(function (a, b) {
            $('#errores').html(error_ajax_jquery(a, b));
...
function continuar_montar_form_editar(datos) {
    $.ajax({
        ...
        .done(function (data) {
            if (data.mensaje_error) {
                ...
                var html_form_editar = "<h2>Editando el producto con cod " + datos["cod"] + "</h2>";
                html_form_editar += "<form onsubmit='event.preventDefault();comprobar_editar(\"" + datos["cod"] + "\");'>";
                html_form_editar += "<p><label for='nombre'>Nombre: </label><input type='text' id='nombre' value='" + (datos["nombre"] ?? "") + "'/></p>"; // El ?? es para saber si existe
                html_form_editar += "<p><label for='nombre_corto'>Nombre Corto: </label><input type='text' id='nombre_corto' value='" + datos["nombre_corto"] + "' required/><span class='error' id='error_nombre_corto'></span></p>";
                html_form_editar += "<p><label for='descripcion'>Descripción: </label><textarea id='descripcion'>" + (datos["descripcion"] ?? "") + "</textarea></p>";
                html_form_editar += "<p><label for='PVP'>PVP: </label><input type='number' id='PVP' min='0.01' step='0.01' value='" + datos["PVP"] + "' required/></p>";
                html_form_editar += "<p><label for='familia'>Seleccione una familia: </label>";
                html_form_editar += "<select id='familia'>";
                $.each(data.familias, function (key, tupla) {
                    if (datos["familia"] == tupla["cod"]) {
                ...
                html_form_editar += "</select></p>";
                html_form_editar += "<p><button onclick='event.preventDefault();volver();'>Volver</button>";
                html_form_editar += "<button>Continuar</button></p>"
    ...
    $.ajax({
        ...
        .done(function (data) {
            if (data.mensaje_error) {
                ...
                $.ajax({
                    url: encodeURI("servicios_rest_key/familia/" + data.producto["familia"]),
                    type: "GET",
                    dataType: "json"
                })
                    .done(function (data2) {
